...
export class AuthenticationService {
    ...
    public login(email:string, password:string) {
        ...
        return this.http.post<AuthResponse>(this.baseUrl + ':' + this.signInEndpoint + '?' + 'key=' + environment.firebase.apiKey, requestBody).pipe(tap(
            (data: AuthResponse) => {
                const currentTime = new Date().getTime();
                const expirationDate = new Date(currentTime + +data.expiresIn * 1000);
                const user = new User(data.email, data.idToken, data.idToken, expirationDate);
                this.user.next(user);
                console.log(user);
            }
        ));
